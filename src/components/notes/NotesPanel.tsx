...
import { Pin, PinOff, Plus, Trash2, Share2, ArrowLeft, Check } from "lucide-react";
...
export function NotesPanel() {
  ...
  const handleContentChange = useCallback(
    (val: string) => {
      ...
      // debounce not needed since we use optimistic updates
    },
...
function NoteEditor({ note, isOwner, onTitleBlur, onContentChange, shareCount }: {
  note: any;
  isOwner: boolean;
  ...
  const handleContentInput = (val: string) => {
    ...
    saveTimerRef.current = setTimeout(() => onContentChange(val), 600);
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-3">
      ...
      <textarea
        ...
        className="w-full min-h-[40vh] bg-transparent text-sm text-foreground leading-relaxed outline-none resize-none placeholder:text-muted-foreground/40"
      />
    </div>
  );
}
